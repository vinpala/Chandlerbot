...
###############################################################################
model_filename = "baseline_glove_small.h5"
...
if os.path.exists(config_file):
    ...
    #hidden_size = 500
    #batch_size = 64
    batch_size = 16 #to tackle memory error
    num_epochs = 30
...
#to beloaded into a frozen Keras Embedding layer
embeddings_index = {}
EMBEDDING_DIM = 100
#UnicodeDecodeError: 'ascii' codec can't decode byte 0xe2 in position 6383: ordinal not in range(128)
#encoding="utf-8" => added to fix this error
#with open(os.path.join(input_data_folder, 'glove.6B.200d.txt'), encoding="utf-8") as f:
with open(os.path.join(input_data_folder, 'glove.6B.100d.txt'), encoding="utf-8") as f:
    for line in f:
        values = line.split()
        word = values[0]
...
X_train = pickle.load(open(os.path.join(input_data_folder,'X_train_small.pickle'), 'rb'))
y_train = pickle.load(open(os.path.join(input_data_folder,'y_train_small.pickle'), 'rb'))
...
X_word_to_idx = np.load(os.path.join(input_data_folder,'X_word_to_idx_small.npy'))
X_idx_to_word = np.load(os.path.join(input_data_folder,'X_idx_to_word_small.npy'))
y_word_to_idx = np.load(os.path.join(input_data_folder,'y_word_to_idx_small.npy'))
y_idx_to_word = np.load(os.path.join(input_data_folder,'y_idx_to_word_small.npy'))
parameters    = np.load(os.path.join(input_data_folder,'parameters_small.npy'))
...
model = models.encoder_decoder(encoder_units,encoder_embed_dim,encoder_layers,
                         decoder_units,decoder_embed_dim,decoder_layers,
                         X_vocab_size,y_vocab_size,X_max_len, y_max_len,learning_rate,pretrained_embed=embedding_matrix,
                         clipvalue=0.5,weight_decay=0.000001,
                         bidirectional=True)
hpo = HPOMetrics()
...
from keras import backend as K
K.clear_session()
# added this for the error :
#2018-12-10T09:05:55Z
#training-I2gCphPmg: File "/usr/local/lib/python3.5/dist-packages/tensorflow/python/client/session.py", line 702, in __del__
#>
#2018-12-10T09:05:55Z
#training-I2gCphPmg: TypeError: 'NoneType' object is not callable
